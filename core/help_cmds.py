...
import nextcord
from Config import images, emojis

class HelpDropdown(nextcord.ui.Select):
    def __init__(self, help_command: "MyHelpCommand", options: list[nextcord.SelectOption]):
        super().__init__(placeholder="• القايمة •", min_values=1, max_values=1, options=options)
...
class MyHelpCommand(commands.MinimalHelpCommand):
    ...
    async def _cog_select_options(self) -> list[nextcord.SelectOption]:
        ...
        options.append(nextcord.SelectOption(
            label="الرئيسية",
            emoji=emojis["home"],
            description="العودة للواجه الرئيسية.",
        ))
        ...
        for cog, command_set in self.get_bot_mapping().items():
            ...
            options.append(nextcord.SelectOption(
                label=cog.qualified_name if cog else "بدون قسم",
                emoji=emoji,
    ...
    async def _help_embed(
        ...
        embed = Embed(title=title, color=nextcord.Color.random())
        if description:
        	embed.description = description + "\n- باقي الاوامر  في slash commnds"
        else:
        	embed.description = f"**باقي الاوامر في slash commands\n- مثال: #help role  | لعرض تفاصيل أكثر ان الامر او اي امر آخر**"
        if set_author:
        ...
        if command_set:
            ...
            for cog, command_set in mapping.items():
                ...
                if not filtered:
                    continue
                name = cog.qualified_name if cog else "بدون قسم"
                emoji = getattr(cog, "COG_EMOJI", None)
